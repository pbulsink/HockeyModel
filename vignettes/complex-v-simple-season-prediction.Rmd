---
title: "Complex Vs Simple Season Prediction"
...
knitr::kable(tail(simple_odds), digits = 3, row.names = FALSE)
```

Compare to the regressed odds:
```{r regressed_odds_table, echo = FALSE, results="asis"}
knitr::kable(tail(simple_odds_regressed), digits = 3, row.names = FALSE)
```
...
knitr::kable(compare_results[,c('Team', 'ComplexPlayoffs','CurrentPlayoffs', 'RegressedPlayoffs')], digits = 3, row.names = FALSE)
```
...
knitr::kable(compare_results[,c('Team', 'ComplexPoints','CurrentPoints', 'RegressedPoints')], digits = 3, row.names = FALSE)
```
