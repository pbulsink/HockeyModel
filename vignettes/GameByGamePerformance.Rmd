...
optDCXI <- function(xi=0.002, results16, schedule16, scores15){
  dc.m <- HockeyModel:::getM(scores = scores15, currentDate = as.Date('2016-10-12'), xi = xi)
  dc.rho <- suppressWarnings(HockeyModel:::getRho(scores = scores15, m = dc.m))
  ...
  for(gameday in unique(schedule16$Date)){
    ...
    dc.m <- HockeyModel:::getM(scores = todayscores, currentDate = as.Date(gameday), xi=xi)
  }
...
#ll opt: 0.02037406 = ll of 0.6701052

optdc<-optim(0.002, optDCXI, results16=results16, schedule16=schedule16, scores15 = scores15, lower=1e-8, upper = 1, control = list(fnscale=-1))
