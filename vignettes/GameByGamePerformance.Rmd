...
for(gameday in unique(schedule16$Date)){
  todayp<-todayDC(today = gameday, schedule = schedule16, m = dc.m, rho = dc.rho)
...
```{r dc1718, echo=FALSE}
dc.m <- HockeyModel:::getM(scores = scores16, currentDate = as.Date('2017-10-02'))
dc.rho <- HockeyModel:::getRho(scores = scores16, m = dc.m)

dcResults<-results16[c(1:1271),]
dcResults$pHome <- NA
dcResults$pAway <- NA

for(gameday in unique(schedule17$Date)){
  todayp<-todayDC(today = gameday, schedule = schedule17, m = dc.m, rho = dc.rho)
  
  dcResults[dcResults$Date == gameday,]$pHome<-todayp$HomeWin
  dcResults[dcResults$Date == gameday,]$pAway<-todayp$AwayWin
  dcResults[dcResults$Date == gameday,]$pDraw<-todayp$Draw
  
  todayscores<-rbind(scores15, results17[results17$Date <= gameday, ])
  dc.m <- HockeyModel:::getM(scores = todayscores, currentDate = gameday)
  dc.rho <- HockeyModel:::getRho(scores = todayscores, m = dc.m)
}
...
And likewise for 2017-2018, we get a Log Loss of `r dc.ll.1718` and accuracy of `r dc.acc.1718`.
