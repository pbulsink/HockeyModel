...
eloRecovery<-function(par = c(8,1505,1300,3,35), scores15=sc15, scores16=sc16, sched16=r16, sched17=r17, a16=stats16, a17=stats17, n = n){
  e15<-HockeyModel:::calculateEloRatings(scores15, k=par[[1]], mean_value = par[[2]], new_teams = par[[3]], regress_strength = par[[3]], home_adv = par[[4]])
  elo.15<-HockeyModel::remainderSeasonELO(scores = NULL, schedule = sched16, elos = e15, nsims = n)$summary_results
  e16<-HockeyModel:::calculateEloRatings(scores16, k=par[[1]], mean_value = par[[2]], new_teams = par[[3]], regress_strength = par[[3]], home_adv = par[[4]], ratings_history = e15)
  elo.16<-HockeyModel::remainderSeasonELO(scores = NULL, schedule = sched17, elos = e16, nsims = n)$summary_results
  return(max(sum(dnorm(a16$Points, mean = elo.15$meanPoints, sd = elo.15$sdPoints), 
             dnorm(a17$Points, mean = elo.16$meanPoints, sd = elo.16$sdPoints), na.rm = TRUE), 0, na.rm = TRUE))
}
elores<-optimParallel(par = c(8, 1505, 1300, 3, 35), fn = eloRecovery, control = list(fnscale=-1, parscale=c(8, 1505, 1300, 3, 35), ndeps = c(1e-2,1e-2,1e-2,1e-2,1e-2)), lower = 0, upper=2000, scores15=sc15, scores16=sc16, sched16=r16, sched17=r17, a16=stats16, a17=stats17, n = n)
