...
elo.15<-simulateSeasonParallel(odds_table = elo.16p, nsims = n, scores = NULL, schedule = r16, cores = cores)$summary_results
...
cat("BT")
btfitted <- readRDS("../data-raw/metricBTFitted.RDS")
bt.16p<-remainderSeasonBT(scores = NULL, schedule = r17, odds = TRUE, fittedBT = btfitted)
bt.16<-simulateSeasonParallel(odds_table = bt.16p, nsims = n, scores = NULL, schedule = r17, cores = cores)$summary_results

cat("BT GD")
btgdfitted <- readRDS("../data-raw/metricBTGDFitted.RDS")
btgd.16p<-remainderSeasonBT(scores = NULL, schedule = r17, odds = TRUE, fittedBT = btgdfitted)
...
library(MLmetrics)
gameLogLoss = list('elo' = LogLoss(y_pred = elo.16p$HomeWin+elo.16p$Draw*elo.16p$HomeWin, y_true = results17$Result),
                   ...
                   'bt' = LogLoss(y_pred = bt.16p$HomeWin+bt.16p$Draw*bt.16p$HomeWin, y_true = results17$Result),
                   'btgd' = LogLoss(y_pred = btgd.16p$HomeWin+btgd.16p$Draw*btgd.16p$HomeWin, y_true = results17$Result),
                   'ml' = 10000)
gameAccuracy = list('elo' = accuracy(pWin = elo.16p$HomeWin+elo.16p$Draw*elo.16p$HomeWin, result = results17$Result),
                    ...
                    'bt' = accuracy(pWin = bt.16p$HomeWin+bt.16p$Draw*bt.16p$HomeWin, result = results17$Result),
                    'btgd' = accuracy(pWin = btgd.16p$HomeWin+btgd.16p$Draw*btgd.16p$HomeWin, result = results17$Result),
                    'ml' = 0)
playoffLogLoss = list('elo' = LogLoss(y_pred = elo.16$Playoffs, y_true = stats17$Playoffs),
                      ...
                      'bt' = LogLoss(y_pred = bt.16$Playoffs, y_true = stats17$Playoffs),
                      'btgd' =LogLoss(y_pred = btgd.16$Playoffs, y_true = stats17$Playoffs),
                      'ml' = 10000)
playoffAccuracy = list('elo' = accuracy(pWin = elo.16$Playoffs, result = stats17$Playoffs),
                       ...
                       'bt' = accuracy(pWin = bt.16$Playoffs, result = stats17$Playoffs),
                       'btgd' = accuracy(pWin = btgd.16$Playoffs, result = stats17$Playoffs),
                       'ml' = 0)
seasonRecovery = list('elo' = recovery(elo.16$meanPoints, elo.16$sdPoints, stats17$Points),
                      ...
                      'bt' = recovery(bt.16$meanPoints, bt.16$sdPoints, stats17$Points),
                      'btgd' = recovery(btgd.16$meanPoints, btgd.16$sdPoints, stats17$Points),
                      'ml' = 0)

results = tibble::tibble('model' = c('elo', 'dc','dc5','dc1','bt', 'btgd', 'ml'), 
