...
updateIterativeRankings <- function(
  ...
  if (is.null(home_xg) || is.null(away_xg)) {
    ...
    home_a <- stats::weighted.mean(
      c(home_goals, home_xg),
      c(1 - attack_mix, attack_mix)
    )
    away_a <- stats::weighted.mean(
      c(away_goals, away_xg),
      c(1 - attack_mix, attack_mix)
    )
    home_d <- stats::weighted.mean(
      c(away_goals, away_xg),
      c(1 - defend_mix, defend_mix)
    )
    away_d <- stats::weighted.mean(
      c(home_goals, home_xg),
      c(1 - defend_mix, defend_mix)
    )
  ...
  va_h <- (gamma + sigma * season_progress) *
    (update[1] - lambda * (home_attack - rho * home_defence))
  vd_h <- (gamma + sigma * season_progress) *
    (update[2] - lambda * (home_defence - rho * home_attack))
  va_a <- (gamma + sigma * season_progress) *
    (update[3] - lambda * (away_attack - rho * away_defence))
  vd_a <- (gamma + sigma * season_progress) *
    (update[4] - lambda * (away_defence - rho * away_attack))
...
getTeamRankings <- function(team, rankings) {
  if (team %in% rankings$Team) {
    ...
    attack <- defence <- 0
    rankings <- dplyr::bind_rows(
      rankings,
      data.frame("Team" = team, "Attack" = 0, "Defence" = 0)
    )
  }
...
iterateGame <- function(
  ...
  new_ranks <- updateIterativeRankings(
    ...
    season_progress = game$SeasonProgress,
    home_xg = game$HomexG,
    away_xg = game$AwayxG,
...
iterateSeason <- function(
  ...
  warm_ups <- scores[
    scores$Date <= as.Date("2021-10-01") & scores$Date > as.Date("2007-08-01"),
  ]
  ...
  for (g in seq_len(nrow(warm_ups))) {
    iters <- iterateGame(
      game = warm_ups[g, ],
      rankings = rankings,
      make_predictions = FALSE,
  ...
  if (verbose) {
    message(
      "Train LL: ",
      logLoss(train$HomeWin, as.integer(train$Result > 0.5))
    )
    ...
    message(
      "Test avg xG/game and G/game: ",
      round(mean(test$TotalxGPred), 3),
      ", ",
      round(mean(test$TotalGoals), 3)
    )
  }
...
optimizeIterative <- function(
  target_model = "wl",
  params = HockeyModel::iterativeParameters
) {
  if (!(target_model %in% c("wl", "WL", "Wl", "wL", "xg", "xG", "XG", "Xg"))) {
    stop(
      "HockeyModel:::optimizeIterative: Target model must be either 'wl' or 'xG'"
    )
  }
...
readIterativePredictions <- function(
  ...
  if (file.exists(file.path(filepath, filename))) {
    ...
    stop(
      "HockeyModel:::read_iterative_rankings: File ",
      filename,
      " doesn't exist at location ",
      filepath,
      "."
    )
  }
...
updateRankingsToToday <- function(
  ...
  params,
  scores = HockeyModel::scores,
  rankings_date = getSeasonStartDate()
) {
  ...
  if (nrow(scores) > 0) {
    ...
    for (g in seq_len(nrow(scores))) {
      iters <- iterateGame(
        ...
        rho = params$rho,
        home_adv = params$home_adv,
        attack_mix = params$attack_mix,
        defend_mix = params$defend_mix
      )
...
getIterativeTable <- function(
  ...
  rankings_xg <- updateRankingsToToday(
    rankings = rankings$rankings_xg,
    params = params_xg,
    scores = scores,
    rankings_date = rankings$date
  )
  rankings_wl <- updateRankingsToToday(
  ...
  results_xg <- getIterativePredictions(
    rankings = rankings_xg,
    params = params_xg,
    date = date,
    schedule = schedule
  )$results
  results_wl <- getIterativePredictions(
    rankings = rankings_wl,
    params = params_wl,
    date = date,
    schedule = schedule
  )$results
...
iterativeOddsTable <- function(
  ...
  for (g in seq_len(nrow(schedule))) {
    i <- iterativeGamePredict(
      home = schedule[g, ]$HomeTeam,
      away = schedule[g, ]$AwayTeam,
      rankings = rankings,
      intercept = params$intercept,
      home_adv = params$home_adv,
      probs_style = "hda"
    ...
    odds_table[nrow(odds_table) + 1, ] <- c(
      ...
      schedule[g, ]$HomeTeam,
      schedule[g, ]$AwayTeam,
      as.numeric(i$Probs[1]),
      as.numeric(i$Probs[3]),
      as.numeric(i$Probs[2])
    )
  }
...
#' @export
getReplacementRankings <- function(
  params = HockeyModel::iterativeParameters,
  save_data = FALSE
) {
  ...
  iterativeRankings <- list(
    "rankings_wl" = rankings_wl,
    "rankings_xg" = rankings_xg,
...
#' @export
getReplacementIterativeParameters <- function(
  params = HockeyModel::iterativeParameters,
  save_data = FALSE
) {
  ...
  params_wl <- list(
    intercept = optim_wl$par[1],
    gamma = optim_wl$par[2],
    lambda = optim_wl$par[3],
    rho = optim_wl$par[4],
    home_adv = optim_wl$par[5],
    attack_mix = optim_wl$par[6],
    defend_mix = optim_wl$par[7],
    sigma = optim_wl$par[8]
  )
  params_xg <- list(
    intercept = optim_xg$par[1],
    gamma = optim_xg$par[2],
    lambda = optim_xg$par[3],
    rho = optim_xg$par[4],
    home_adv = optim_xg$par[5],
    attack_mix = optim_xg$par[6],
    defend_mix = optim_xg$par[7],
    sigma = optim_xg$par[8]
  )
